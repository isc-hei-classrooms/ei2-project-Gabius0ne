"""
train_lgbm_v12_last_year.py
===========================
Script DIAGNOSTIQUE : entraîne uniquement sur la dernière année
avant le test set pour vérifier l'hypothèse de croissance PV.

Si le modèle est meilleur en JOUR avec 1 an de train (capacité PV
proche du test) qu'avec 3 ans (capacité PV moyenne trop basse),
c'est la confirmation que LightGBM ne peut pas extrapoler la capacité PV.

Split :
  - Train : ~1 an avant le val set
  - Val   : 20% standard (pour Optuna)
  - Test  : 20% standard (dernier 20%)

Garde le même test set que le script normal pour comparaison directe.

Sorties :
  DATA/models12_lastyear/
"""
...
# Groupes identiques au script normal
...
# ─────────────────────────────────────────────
# 2. SPLIT — même test/val que le script normal,
#    mais train = seulement la dernière année avant val
# ─────────────────────────────────────────────

split_train_full = int(n_samples * TRAIN_RATIO)
split_val        = int(n_samples * (TRAIN_RATIO + VAL_RATIO))

# Val et Test identiques au script normal
X_val  = X_arr[split_train_full:split_val]
Y_val  = Y_arr[split_train_full:split_val]
X_test = X_arr[split_val:]
Y_test = Y_arr[split_val:]
B_test = B_arr[split_val:]
dates_test = dates[split_val:]

# Train : dernière année avant le val set (~365 jours)
TRAIN_LAST_N_DAYS = 365
train_start = max(0, split_train_full - TRAIN_LAST_N_DAYS)
X_train = X_arr[train_start:split_train_full]
Y_train = Y_arr[train_start:split_train_full]

print(f"\n=== Split LAST YEAR ===")
print(f"  Train complet dispo : {split_train_full} jours ({dates[0]} → {dates[split_train_full-1]})")
print(f"  Train UTILISÉ : {X_train.shape[0]} jours ({dates[train_start]} → {dates[split_train_full-1]})")
print(f"  Jours ignorés : {train_start} (les plus anciens)")
print(f"  Val   : {split_val - split_train_full} jours ({dates[split_train_full]} → {dates[split_val-1]})")
...
# ─────────────────────────────────────────────
# 4. ENTRAÎNEMENT FINAL — train(1an)+val → test
# ─────────────────────────────────────────────

print(f"\n=== Entraînement final (96 modèles, train=dernière année) ===")
...
print(f"\n=== Résultats globaux (train=dernière année, excl. 13-15 sept) ===")
print(f"  Train : {X_train.shape[0]} jours | Test : {exclude_mask.sum()} jours (exclu {n_excluded})")
...
# ─────────────────────────────────────────────
# 7. DIAGNOSTIC BIAIS (signe corrigé)
# ─────────────────────────────────────────────
...
for month_key in sorted(monthly_bias.keys()):
    ...
    n = len(vals)
    # réel - prédit > 0 → prédit trop bas → surestime PV (soustrait trop)
    # réel - prédit < 0 → prédit trop haut → sous-estime PV (soustrait pas assez)
    if mean_bias > 0.05:
