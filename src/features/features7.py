...
Sorties :
  DATA/processed/X_features_v7.parquet
  DATA/processed/Y_target_v7.parquet
  DATA/processed/B_baseline_v7.parquet
"""
...
def build_features(
    ...
    # ── NOUVEAU v7: Features PV agrégées J+1 et J
    # Condense le signal PV en quelques features fortes pour que le modèle
    # voie directement "beaucoup/peu de PV" sans assembler 144 features diluées.
    for prefix in ["pred", "predJ"]:
        # Total PV prédit sur 24h
        pv_24h = [features.get(f"{prefix}_pv_MW_t{h:02d}") for h in range(24)]
        pv_clean = [v for v in pv_24h if v is not None]
        features[f"{prefix}_pv_total"] = sum(pv_clean) if pv_clean else None

        # Total PV prédit heures diurnes (06h–20h UTC)
        pv_day = [features.get(f"{prefix}_pv_MW_t{h:02d}") for h in range(6, 20)]
        pv_day_clean = [v for v in pv_day if v is not None]
        features[f"{prefix}_pv_day"] = sum(pv_day_clean) if pv_day_clean else None

        # Irradiance moyenne diurne toutes stations (06h–20h UTC)
        irr_day_vals = []
        for h in range(6, 20):
            for station in STATIONS:
                v = features.get(f"{prefix}_glob_rad_{station}_t{h:02d}")
                if v is not None:
                    irr_day_vals.append(v)
        features[f"{prefix}_glob_rad_mean_day"] = (
        ...
        for h in range(24):
            ...
            if len(station_vals) >= 2:
                ...
                features[f"{prefix}_glob_rad_std_stations_t{h:02d}"] = var_v ** 0.5
            else:
                features[f"{prefix}_glob_rad_std_stations_t{h:02d}"] = None
...
def main():
    ...
    print(f"\n=== Construction features v7 : {first_day} → {last_day} ({len(all_dates)} jours) ===")
    ...
    X.write_parquet(OUT / "X_features_v7.parquet")
    Y.write_parquet(OUT / "Y_target_v7.parquet")
    B.write_parquet(OUT / "B_baseline_v7.parquet")

    print(f"\n✓ X_features_v7 : {X.shape[0]} jours × {X.shape[1]} colonnes")
    print(f"✓ Y_target_v7   : {Y.shape[0]} jours × {Y.shape[1]} colonnes")
    print(f"✓ B_baseline_v7 : {B.shape[0]} jours × {B.shape[1]} colonnes")
