...
def build_features(
    ...
    # ── Tendance temporelle (croissance PV installé)
    # Le PV installé sur le territoire Oiken augmente d'année en année.
    # Le load normalisé 09h-15h passe de +0.16 (2023) à -0.81 (2025).
    # Sans feature temporelle, le modèle apprend un creux PV "moyen" sur 3 ans
    # et sous-estime systématiquement le PV de 2025.
    # days_since_start : compteur linéaire depuis le début des données
    # year_frac : année fractionnelle (2023.5 = mi-2023)
    # Ces features permettent au modèle d'apprendre que le creux PV s'approfondit.
    EPOCH = date(2022, 10, 1)  # début des données Oiken
    features["days_since_start"] = (target_date - EPOCH).days
    features["year_frac"] = target_date.year + (doy - 1) / 365.0
...
def main():
    ...
    X.write_parquet(OUT / "X_features_v9.parquet")
    Y.write_parquet(OUT / "Y_target_v9.parquet")
    B.write_parquet(OUT / "B_baseline_v9.parquet")

    print(f"\n✓ X_features_v9 : {X.shape[0]} jours × {X.shape[1]} colonnes")
    print(f"✓ Y_target_v9   : {Y.shape[0]} jours × {Y.shape[1]} colonnes")
    print(f"✓ B_baseline_v9 : {B.shape[0]} jours × {B.shape[1]} colonnes")
