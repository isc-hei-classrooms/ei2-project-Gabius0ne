...
Sorties :
  ...
  DATA/processed/Y_target_v10.parquet
  DATA/processed/B_baseline_v10.parquet
"""
...
# Variables d'incertitude irradiance (nouveau v10)
...
PV_PERF_RATIO  = 0.75      # performance ratio
...
def _get_pv_capacity_proxy(d: date) -> float:
    """
    Proxy de la puissance PV cumulée installée sur le territoire Oiken (MWp).
    
    Basé sur les données nationales suisses (Swissolar/IEA-PVPS) :
      Fin 2022: 4.65 GW (+1.08 GW en 2022)
      Fin 2023: 6.20 GW (+1.55 GW en 2023)
      Fin 2024: 8.00 GW (+1.78 GW en 2024)
      2025: prévision +1.51 GW (Swissolar)
    
    Interpolation linéaire par mois, rapportée au territoire Oiken
    (108 MWp en Mars 2026, croissance proportionnelle à la Suisse).
    
    En production: remplacer par la vraie valeur d'Oiken si disponible.
    """
    from datetime import date as _date
    
    # Points d'ancrage: (date, capacité nationale GW)
    anchors = [
        (_date(2022, 10, 1), 4.65),
        (_date(2022, 12, 31), 4.65),
        (_date(2023, 12, 31), 6.20),
        (_date(2024, 12, 31), 8.00),
        (_date(2025, 12, 31), 9.51),  # 8.00 + 1.51 prévision
    ]
    ...
    # Rapporter au territoire Oiken (proportionnel)
    OIKEN_BASE_MWP = 55.0       # MWp estimé oct 2022
    NATIONAL_BASE_GW = 4.65      # GW oct 2022
...
def get_correct_horizon_jp1(h_utc: int) -> int | None:
    ...
    Si aucun horizon valide n'est disponible (h_utc=22,23 où horizon_min
    dépasse le max du pattern), retourne None pour éviter le leakage.
    Ces heures (00h-01h Zurich) auront des features à None.
    """
    mod = h_utc % 3
    ...
    if valid:
        ...
        # Aucun horizon disponible sans dépasser le deadline → None (anti-leakage)
        return None
...
def extract_pred_vector(
    ...
    Paramètres:
      ...
      horizon_func : fonction h_utc → horizon correct (différent pour J vs J+1)
                     Peut retourner None si aucun horizon valide (anti-leakage).
    ...
    for var in all_vars:
        ...
        for station in STATIONS:
            ...
            for h_utc in range(24):
                ...
                # Si horizon est None → aucun horizon valide sans leakage → feature à None
                if horizon is None:
                    if is_cyclic:
...
def build_features(
    ...
    # NOUVEAU v10: inclut glob_rad_q10, q90, stde (incertitude irradiance)
    ...
    # v10: SANS extra_vars PV — importance groupée PV J = 0.6 (bruit pur)
    features.update(extract_pred_vector(
        meteo_utc, day_j,
    ...
    # ── Features PV agrégées J+1 uniquement (PV J supprimé en v10)
    ...
    # ── v10: Incertitude PV agrégée (spread q90-q10 moyen sur heures diurnes)
    spread_vals = []
    for h in range(6, 20):
        for station in STATIONS:
    ...
    # ── Proxy puissance PV installée territoire Oiken (MWp)
    # Basé sur la croissance nationale suisse (Swissolar/IEA-PVPS),
    # interpolée mensuellement et rapportée au territoire Oiken.
    # Données nationales: 4.65 GW (fin 2022) → 6.20 (fin 2023) → 8.00 (fin 2024)
    # Oiken base: 108 MWp (oct 2022), croissance proportionnelle.
    # Feature bornée et physiquement significative — pas d'extrapolation.
    # En production: mettre à jour avec la valeur connue de puissance installée.
