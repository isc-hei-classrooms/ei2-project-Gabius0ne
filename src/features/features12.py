"""
pipeline_features_v12.py

Changements par rapport à v11 :

  SUPPRESSION pred_pv_MW_tXX / predJ_pv_MW_tXX / pred_pv_total / pred_pv_day
    Ces features utilisaient un facteur fixe (surface × PR) pour convertir
    l'irradiance en production PV. Ce facteur encode une capacité PV constante
    alors que la puissance installée a doublé sur 3 ans (55 → ~108 MWp).
    Le modèle s'appuyait sur ces features biaisées → sous-estimation PV systématique.

  AJOUT features d'interaction irradiance × capacité PV récente :
    pred_pv_adj_30j_tXX = mean(glob_rad stations) × pv_yield_30j  (24h J+1)
    pred_pv_adj_90j_tXX = mean(glob_rad stations) × pv_yield_90j  (24h J+1)
    pred_pv_adj_30j_day = somme diurne (06h-20h) de pred_pv_adj_30j
    pred_pv_adj_90j_day = somme diurne (06h-20h) de pred_pv_adj_90j
    
    Ces features donnent directement au modèle le produit irradiance × capacité
    récente, ce que LightGBM ne peut pas reconstruire efficacement par splits
    (il ne peut pas extrapoler au-delà des combinaisons vues en training).
    
    Anti-leakage : pv_yield basé sur J-1 et avant, irradiance = prévisions J+1 légitime.

Sorties :
  DATA/processed/X_features_v12.parquet
  DATA/processed/Y_target_v12.parquet
  DATA/processed/B_baseline_v12.parquet
"""
...
def extract_pred_vector(
    ...
    """
    v12 : ne génère PLUS les features pred_pv_MW_tXX (facteur fixe supprimé).
    Les features d'interaction irradiance × capacité sont calculées séparément
    dans build_features() avec pv_yield_30j/90j.
    """
    ...
    if len(day_utc) == 0:
        ...
        # v12 : plus de pred_pv_MW_tXX ici
        return features
    ...
    # v12 : on ne calcule PLUS pred_pv_MW_tXX ici (facteur fixe supprimé)
    # Les interactions irradiance × capacité sont dans build_features()

    return features


# ─────────────────────────────────────────────
# PV YIELD RATIO (v11, conservé en v12)
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# v12 : FEATURES D'INTERACTION IRRADIANCE × CAPACITÉ PV
# ─────────────────────────────────────────────

def compute_pv_interaction_features(features: dict, prefix: str = "pred") -> dict:
    """
    Crée des features d'interaction explicite :
      irradiance_prévue_moyenne_stations × pv_yield_30j (et 90j)
    
    Remplace les anciennes pred_pv_MW_tXX qui utilisaient un facteur fixe.
    
    Le produit irradiance × yield donne directement au modèle une estimation
    de production PV ajustée à la capacité récente, sans que LightGBM ait
    besoin de reconstruire cette multiplication par des splits conditionnels.
    
    Calcul par heure UTC 0-23 pour les prévisions J+1 (prefix="pred").
    """
    interaction_feats = {}
    ...
    for h_utc in range(24):
        # Moyenne irradiance prévue sur les 6 stations
        irr_vals = []
        for station in STATIONS:
            v = features.get(f"{prefix}_glob_rad_{station}_t{h_utc:02d}")
            if v is not None:
                irr_vals.append(v)
        ...
        # Interaction avec pv_yield_30j
        if irr_mean is not None and pv_yield_30j is not None:
            interaction_feats[f"{prefix}_pv_adj_30j_t{h_utc:02d}"] = irr_mean * pv_yield_30j
        else:
            interaction_feats[f"{prefix}_pv_adj_30j_t{h_utc:02d}"] = None
        
        # Interaction avec pv_yield_90j
        if irr_mean is not None and pv_yield_90j is not None:
            interaction_feats[f"{prefix}_pv_adj_90j_t{h_utc:02d}"] = irr_mean * pv_yield_90j
        else:
            interaction_feats[f"{prefix}_pv_adj_90j_t{h_utc:02d}"] = None
    
    # Agrégés diurnes (06h-20h UTC)
    for label, yield_key in [("30j", "pv_yield_30j"), ("90j", "pv_yield_90j")]:
        day_vals = []
        for h in range(6, 20):
            v = interaction_feats.get(f"{prefix}_pv_adj_{label}_t{h:02d}")
            if v is not None:
                day_vals.append(v)
        interaction_feats[f"{prefix}_pv_adj_{label}_day"] = sum(day_vals) if day_vals else None
...
def build_features(
    ...
    # ── Prévisions J+1 (v12 : sans pred_pv_MW)
    ...
    # ── Prévisions J (v12 : sans predJ_pv_MW)
    features.update(extract_pred_vector(
        meteo_utc, day_j,
    ...
    # ── PV yield ratio glissant (v11, conservé)
    features.update(compute_pv_yield_ratios(oiken, meteo_zurich, target_date))

    # ── v12 : Features d'interaction irradiance × capacité PV
    # On les calcule APRÈS pv_yield car elles en dépendent
    features.update(compute_pv_interaction_features(features, prefix="pred"))

    # ── Features PV agrégées J+1 (v12 : refait avec irradiance brute, sans facteur fixe)
    # On garde glob_rad_mean_day et glob_rad_std_stations comme features d'irradiance pure
    irr_day_vals = []
    for h in range(6, 20):
        for station in STATIONS:
            v = features.get(f"pred_glob_rad_{station}_t{h:02d}")
            if v is not None:
    ...
    for h in range(24):
        ...
        if len(station_vals) >= 2:
            ...
            features[f"pred_glob_rad_std_stations_t{h:02d}"] = var_v ** 0.5
        else:
            features[f"pred_glob_rad_std_stations_t{h:02d}"] = None

    spread_vals = []
    for h in range(6, 20):
        for station in STATIONS:
            q90 = features.get(f"pred_glob_rad_q90_{station}_t{h:02d}")
            q10 = features.get(f"pred_glob_rad_q10_{station}_t{h:02d}")
...
def main():
    ...
    print(f"\n=== Construction features v12 : {first_day} → {last_day} ({len(all_dates)} jours) ===")
    ...
    X.write_parquet(OUT / "X_features_v12.parquet")
    Y.write_parquet(OUT / "Y_target_v12.parquet")
    B.write_parquet(OUT / "B_baseline_v12.parquet")

    print(f"\n✓ X_features_v12 : {X.shape[0]} jours × {X.shape[1]} colonnes")
    print(f"✓ Y_target_v12   : {Y.shape[0]} jours × {Y.shape[1]} colonnes")
    print(f"✓ B_baseline_v12 : {B.shape[0]} jours × {B.shape[1]} colonnes")

    # Vérifier les nouvelles features
    check_cols = [
        "pv_yield_30j", "pv_yield_90j",
        "solar_remote_max_30j", "solar_remote_max_90j",
        "pv_capacity_MWp",
        "pred_pv_adj_30j_t12", "pred_pv_adj_90j_t12",
        "pred_pv_adj_30j_day", "pred_pv_adj_90j_day",
    ]
    ...
    # Vérifier que les anciennes features PV à facteur fixe sont absentes
    old_pv_cols = [c for c in X.columns if c.startswith("pred_pv_MW_") or c.startswith("predJ_pv_MW_")]
    if old_pv_cols:
        print(f"\n⚠ ATTENTION : anciennes features pred_pv_MW encore présentes : {old_pv_cols}")
    else:
        print(f"\n✓ Anciennes features pred_pv_MW supprimées (facteur fixe éliminé)")

    # Compter les features PV interaction
    new_pv_cols = [c for c in X.columns if "pv_adj" in c]
    print(f"✓ Nouvelles features d'interaction PV : {len(new_pv_cols)} ({', '.join(new_pv_cols[:5])}...)")
