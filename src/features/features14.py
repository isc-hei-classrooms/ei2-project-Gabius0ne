"""
pipeline_features_v14.py
========================

Pipeline de construction des features journalières pour le forecast
day-ahead Oiken (96 pas de 15 min).

Identique à v13, avec l'ajout d'une feature `pv_normalizer_90j` utilisée
par le training v14 pour normaliser la cible (per-unit) sur les pas
diurnes uniquement.
...
(11h UTC+1 le jour J) :
  ...
  - [v14] pv_normalizer_90j → facteur de normalisation per-unit pour la
    cible diurne, calculé comme moyenne de solar_remote entre 10h et 17h
    UTC sur les 90 jours précédents
...
CHANGEMENT v13 → v14
--------------------
Ajout de la feature `pv_normalizer_90j` calculée dans compute_pv_yield_ratios.

  Définition : moyenne de solar_remote sur les heures 10h-17h UTC des
  90 jours précédant J-2 (= day_jm1 dans la convention du code).

  Motivation : le parc PV Oiken croît dans le temps (de ~55 MWp en 2022
  à ~112 MWp estimés en 2026). L'amplitude PV dans la cible load_net
  n'est donc pas stationnaire. Une feature de référence calculée sur
  une fenêtre récente permet, dans le training, de normaliser la cible
  diurne en per-unit (comme on passe en pu dans les calculs
  électrotechniques), puis de dénormaliser pour évaluer dans l'espace
  original comparable à la baseline Oiken.

  Fenêtre diurne 10h-17h UTC : cohérente avec DAY_STEPS (indices 40-67)
  du training. Évite le bruit nocturne de solar_remote (offset suspect
  identifié en v13 diagnostic).

  Moyenne plutôt que max : plus robuste aux outliers que le max utilisé
  dans pv_yield_*.

  Anti-leakage : fenêtre finissant strictement à day_jm1 = target_date - 2,
  aucune donnée ≥ target_date - 1 n'est touchée.

Tous les autres aspects (STATIONS_PLAINE, STATIONS_WIND, HOURS_DIURNAL,
structure des features, règle anti-leakage MeteoSuisse) sont identiques
à v13.

SORTIES
-------
  DATA/processed/X_features_v14.parquet   — features (1 ligne/jour, +1 col vs v13)
  DATA/processed/Y_target_v14.parquet     — cibles load 96 pas (1 ligne/jour)
  DATA/processed/B_baseline_v14.parquet   — prévisions Oiken 96 pas (pour benchmark)
"""
...
HOURS_DIURNAL   = list(range(6, 20))  # 06h-19h UTC (irradiance et vent)
...
def main():
    ...
    print(f"\n=== Construction features v14 : {first_day} → {last_day} ({len(all_dates)} jours) ===")
    ...
    X.write_parquet(OUT / "X_features_v14.parquet")
    Y.write_parquet(OUT / "Y_target_v14.parquet")
    B.write_parquet(OUT / "B_baseline_v14.parquet")
    ...
    # Stats spécifiques v14 sur pv_normalizer_90j
    if "pv_normalizer_90j" in X.columns:
        s = X["pv_normalizer_90j"]
        n_null = s.null_count()
        print(f"\n  pv_normalizer_90j :")
        print(f"    {n_null} jours avec normalizer manquant (seront skipés à l'entraînement v14)")
        print(f"    Représente {n_null/len(s)*100:.1f}% du dataset")

    # Vérifier l'absence de features supprimées en v13
    ...
    print(f"  PV (yield/capacity/interaction/normalizer) : {n_pv} features")
