"""
pipeline_features_v5.py
=======================
Améliorations par rapport à v4 :

1. VECTEUR MÉTÉO 24H VIA H13/H14/H15 UNIQUEMENT
   Pour chaque heure H de J+1 (00h à 23h UTC), on prend la prévision
   la plus récente disponible avant 11h le jour J :
     H % 3 == 1 → pred_var_h13_Station (01h, 04h, 07h, 10h, 13h, 16h, 19h, 22h)
     H % 3 == 2 → pred_var_h14_Station (02h, 05h, 08h, 11h, 14h, 17h, 20h, 23h)
     H % 3 == 0 → pred_var_h15_Station (00h, 03h, 06h, 09h, 12h, 15h, 18h, 21h)
   Résultat : 24 valeurs propres par variable par station
   au lieu de 576 colonnes diluées (24 horizons × 24 heures).

2. IS_RAMADAN_HOUR
   Feature = 1 uniquement si la date est pendant le Ramadan ET
   l'heure cible est entre 21h et 05h (horaire nocturne).
   Permet au modèle de distinguer "nuit de Ramadan" (impact fort)
   de "jour de Ramadan" (impact faible).
   Note : is_ramadan_hour est calculé pour target_date (J+1),
   pas pour J — anti-leakage respecté car c'est une info calendaire connue.

3. FEATURE PV PRÉVU J+1
   Pour chaque heure H UTC de J+1, on calcule la production PV estimée
   sur le territoire Oiken en MW :
     pred_pv_MW_t{h} = moyenne(pred_glob_rad sur 6 stations) × 540_000 m² × 0.80 PR / 1_000_000
   Facteur dérivé de : puissance installée Oiken = 108 MWp, densité = 200 Wc/m²
   → surface = 540_000 m², performance ratio = 0.80
   ANTI-LEAKAGE : basé sur pred_glob_rad h13/h14/h15, disponible avant 11h le jour J.

4. VECTORISATION POLARS
   Remplacement de iter_rows() par des opérations sur colonnes entières.
   Gain estimé : 10x à 50x sur la vitesse de génération des features.

ANTI-LEAKAGE strict maintenu :
  - Load historique  : J-1 à J-7 uniquement
  - Météo réelle     : J-1 complet + J matin ≤10h uniquement
  - Prévisions météo : h13/h14/h15 sur J+1 (émises avant 11h le jour J)
  - PV prévu J+1     : basé sur pred_glob_rad h13/h14/h15 × facteur conversion
  - is_ramadan_hour  : info calendaire connue à l'avance

Sorties :
  DATA/processed/X_features_v5.parquet
  DATA/processed/Y_target_v5.parquet
  DATA/processed/B_baseline_v5.parquet
"""

import math
import polars as pl
import numpy as np
from pathlib import Path
from datetime import timedelta, date

# ─────────────────────────────────────────────
# CONFIGURATION
# ─────────────────────────────────────────────
BASE    = Path(r"C:\Users\gab1a\OneDrive\Documents\energyinfo2\DATA")
CSV     = BASE / "oiken-data.csv"
METEO   = BASE / "meteo_multistation_v3.parquet"
OUT     = BASE / "processed"
OUT.mkdir(exist_ok=True)

PROD_COLS = [
    "solar_central_valais",
    "solar_sion",
    "solar_sierre",
    "solar_remote",
]

LOAD_HISTORY_DAYS = list(range(2, 9))  # J-1 à J-7 (delta=2 à 8 par rapport à target_date)

REAL_METEO_VARS = ["temp_2m", "glob_rad", "pressure", "relhum_2m"]

STATIONS = [
    "Pully", "Sion", "Visp", "Montana",
    "Col_du_Grand_St-Bernard", "Les_Attelas",
]

# Variables météo pour le vecteur 24h (h13/h14/h15 uniquement)
PRED_VARS = ["temp", "glob_rad", "pressure", "relhum", "precip", "sunshine", "wind_speed", "wind_dir"]

# Horizons retenus : h13, h14, h15 — les plus récents avant 11h le jour J
# H % 3 == 1 → h13, H % 3 == 2 → h14, H % 3 == 0 → h15
HORIZON_MAP = {1: 13, 2: 14, 0: 15}

# Facteur de conversion irradiance → production PV territoire Oiken
# Puissance installée Oiken : 108 MWp
# Densité moyenne panneau   : 200 Wc/m² → surface = 108e6 / 200 = 540_000 m²
# Performance ratio (PR)    : 0.80 (pertes câbles, onduleur, température)
# PV_MW = irradiance [W/m²] × 540_000 [m²] × 0.80 / 1_000_000
PV_SURFACE_M2  = 540_000   # m²
PV_PERF_RATIO  = 0.80      # performance ratio

# Dates du Ramadan par année (début inclus, fin inclus)
RAMADAN_DATES = {
    2022: (date(2022, 4,  2), date(2022, 5,  1)),
    2023: (date(2023, 3, 23), date(2023, 4, 20)),
    2024: (date(2024, 3, 11), date(2024, 4,  9)),
    2025: (date(2025, 3,  1), date(2025, 3, 29)),
    2026: (date(2026, 2, 18), date(2026, 3, 19)),
}

FERIES = {
    date(2022, 11,  1), date(2022, 12,  8), date(2022, 12, 25), date(2022, 12, 26),
    date(2023,  1,  1), date(2023,  1,  2), date(2023,  3, 19), date(2023,  4,  7),
    date(2023,  4, 10), date(2023,  5, 18), date(2023,  5, 29), date(2023,  6,  8),
    date(2023,  8,  1), date(2023, 11,  1), date(2023, 12,  8), date(2023, 12, 25), date(2023, 12, 26),
    date(2024,  1,  1), date(2024,  1,  2), date(2024,  3, 19), date(2024,  3, 29),
    date(2024,  4,  1), date(2024,  5,  9), date(2024,  5, 20), date(2024,  5, 30),
    date(2024,  8,  1), date(2024, 11,  1), date(2024, 12,  8), date(2024, 12, 25), date(2024, 12, 26),
    date(2025,  1,  1), date(2025,  1,  2), date(2025,  3, 19), date(2025,  4, 18),
    date(2025,  4, 21), date(2025,  5, 29), date(2025,  6,  9), date(2025,  6, 19),
    date(2025,  8,  1), date(2025, 11,  1), date(2025, 12,  8), date(2025, 12, 25), date(2025, 12, 26),
}


# ─────────────────────────────────────────────
# HELPERS CALENDAIRES
# ─────────────────────────────────────────────

def is_ramadan(d: date) -> bool:
    """Retourne True si la date est pendant le Ramadan."""
    for start, end in RAMADAN_DATES.values():
        if start <= d <= end:
            return True
    return False


def ramadan_night_hours(d: date) -> list[int]:
    """
    Retourne les heures UTC nocturnes du Ramadan pour une date donnée.
    Heures nocturnes = 21h–23h + 00h–05h (heure locale ≈ UTC+1/+2)
    On utilise UTC car le modèle travaille en UTC pour les prévisions.
    """
    if not is_ramadan(d):
        return []
    return list(range(0, 6)) + list(range(20, 24))  # 00h-05h et 20h-23h UTC


# ─────────────────────────────────────────────
# 1. CHARGEMENT
# ─────────────────────────────────────────────

def load_oiken(path: Path) -> pl.DataFrame:
    df = pl.read_csv(
        path,
        try_parse_dates=False,
        null_values=["#N/A", "N/A", "NA", ""],
        schema_overrides={
            "central valais solar production [kWh]": pl.Float64,
            "sion area solar production [kWh]":      pl.Float64,
            "sierre area production [kWh]":          pl.Float64,
            "remote solar production [kWh]":         pl.Float64,
        }
    )
    df = df.with_columns(
        pl.col("timestamp")
          .str.strptime(pl.Datetime("us"), "%d.%m.%Y %H:%M")
          .dt.replace_time_zone("Europe/Zurich", ambiguous="earliest", non_existent="null")
          .alias("timestamp")
    ).rename({
        "standardised load [-]":                "load",
        "standardised forecast load [-]":        "load_forecast_oiken",
        "central valais solar production [kWh]": "solar_central_valais",
        "sion area solar production [kWh]":      "solar_sion",
        "sierre area production [kWh]":          "solar_sierre",
        "remote solar production [kWh]":         "solar_remote",
    }).sort("timestamp")
    print(f"  Oiken : {len(df):,} lignes | {df['timestamp'].drop_nulls()[0]} → {df['timestamp'][-1]}")
    return df


def load_meteo(path: Path) -> tuple[pl.DataFrame, pl.DataFrame, list[str]]:
    """
    Retourne :
      - df_utc    : timestamps UTC — pour pred_vector_24h
      - df_zurich : timestamps Europe/Zurich — pour mesures réelles passées
      - real_cols : colonnes de mesures réelles disponibles
    """
    df_utc = pl.read_parquet(path).sort("timestamp")
    df_zurich = df_utc.with_columns(
        pl.col("timestamp").dt.convert_time_zone("Europe/Zurich")
    )
    all_cols  = [c for c in df_utc.columns if c != "timestamp"]
    real_cols = [
        f"{var}_{station}"
        for var in REAL_METEO_VARS
        for station in STATIONS
        if f"{var}_{station}" in all_cols
    ]
    print(f"  Météo : {len(df_utc):,} lignes | {len(real_cols)} colonnes réelles")
    return df_utc, df_zurich, real_cols


# ─────────────────────────────────────────────
# 2. HELPERS
# ─────────────────────────────────────────────

def get_day_slice(df: pl.DataFrame, day: date) -> pl.DataFrame:
    start = pl.datetime(day.year, day.month, day.day, 0, 0, 0, time_zone="Europe/Zurich")
    end   = pl.datetime(day.year, day.month, day.day, 23, 59, 59, time_zone="Europe/Zurich")
    return df.filter((pl.col("timestamp") >= start) & (pl.col("timestamp") <= end))


def get_morning_slice(df: pl.DataFrame, day: date, until_hour: int = 10) -> pl.DataFrame:
    start = pl.datetime(day.year, day.month, day.day, 0, 0, 0, time_zone="Europe/Zurich")
    end   = pl.datetime(day.year, day.month, day.day, until_hour, 0, 0, time_zone="Europe/Zurich")
    return df.filter((pl.col("timestamp") >= start) & (pl.col("timestamp") <= end))


def series_stats(series: pl.Series, prefix: str) -> dict:
    vals = series.drop_nulls()
    if len(vals) == 0:
        return {f"{prefix}_mean": None, f"{prefix}_max": None,
                f"{prefix}_min": None, f"{prefix}_std": None}
    return {
        f"{prefix}_mean": float(vals.mean()),
        f"{prefix}_max":  float(vals.max()),
        f"{prefix}_min":  float(vals.min()),
        f"{prefix}_std":  float(vals.std()),
    }


def hourly_profile(df_day: pl.DataFrame, col: str, prefix: str) -> dict:
    result = {}
    for h in range(24):
        hour_vals = df_day.filter(pl.col("timestamp").dt.hour() == h)[col].drop_nulls()
        result[f"{prefix}_h{h:02d}"] = float(hour_vals.mean()) if len(hour_vals) > 0 else None
    return result


def real_meteo_stats(df_slice: pl.DataFrame, real_cols: list[str], prefix: str) -> dict:
    """
    Stats (mean/max/min) des mesures réelles météo sur une tranche passée.
    ANTI-LEAKAGE : uniquement J-1 complet ou J matin ≤10h.
    Vectorisé : on extrait mean/max/min sur toute la colonne en une opération.
    """
    features = {}
    for col in real_cols:
        if col in df_slice.columns:
            vals = df_slice[col].drop_nulls()
            if len(vals) > 0:
                features[f"{prefix}_{col}_mean"] = float(vals.mean())
                features[f"{prefix}_{col}_max"]  = float(vals.max())
                features[f"{prefix}_{col}_min"]  = float(vals.min())
            else:
                features[f"{prefix}_{col}_mean"] = None
                features[f"{prefix}_{col}_max"]  = None
                features[f"{prefix}_{col}_min"]  = None
    return features


def pred_vector_24h(
    meteo_utc: pl.DataFrame,
    target_date: date,
) -> dict:
    """
    Construit le vecteur de prévisions météo heure par heure pour J+1.

    Pour chaque heure H UTC (00h à 23h) de J+1, on prend la prévision
    la plus récente disponible avant 11h le jour J :
      H % 3 == 1 → colonne pred_var_h13_Station au timestamp H
      H % 3 == 2 → colonne pred_var_h14_Station au timestamp H
      H % 3 == 0 → colonne pred_var_h15_Station au timestamp H

    VECTORISÉ : on filtre J+1 une seule fois, puis on extrait les valeurs
    par opération Polars sur colonnes entières (pas d'iter_rows).

    ANTI-LEAKAGE : h13/h14/h15 correspondent à des runs émis entre
    J-2 22h et J 10h — tous avant la soumission à 11h le jour J.

    Résultat : 24 valeurs par variable par station (192 features pour
    6 stations × 8 variables) au lieu de 576 colonnes diluées.
    """
    features = {}

    # Filtrer J+1 en UTC — une seule fois pour toutes les variables
    start = pl.datetime(target_date.year, target_date.month, target_date.day,
                        0, 0, 0, time_zone="UTC")
    end   = pl.datetime(target_date.year, target_date.month, target_date.day,
                        23, 59, 59, time_zone="UTC")
    day_utc = meteo_utc.filter(
        (pl.col("timestamp") >= start) & (pl.col("timestamp") <= end)
    )

    if len(day_utc) == 0:
        # Pas de données météo pour ce jour — tout à None
        for var in PRED_VARS:
            for station in STATIONS:
                for h in range(24):
                    features[f"pred_{var}_{station}_t{h:02d}"] = None
        return features

    # VECTORISÉ : extraire les heures UTC une seule fois
    # Polars extrait toute la colonne d'heures en C++ — pas de boucle Python
    hours_utc = day_utc["timestamp"].dt.hour().to_list()

    # Construire un index heure → position dans day_utc
    hour_to_idx = {}
    for idx, h in enumerate(hours_utc):
        if h not in hour_to_idx:
            hour_to_idx[h] = idx

    for var in PRED_VARS:
        for station in STATIONS:
            # Récupérer les 3 colonnes h13/h14/h15 en une opération
            col_map = {
                horizon: f"pred_{var}_h{horizon}_{station}"
                for horizon in [13, 14, 15]
            }

            # Vérifier quelles colonnes existent
            existing = {h: col for h, col in col_map.items() if col in day_utc.columns}

            # VECTORISÉ : extraire les valeurs de chaque colonne d'un coup
            col_values = {}
            for horizon, col in existing.items():
                col_values[horizon] = day_utc[col].to_list()

            # Pour chaque heure H de J+1, prendre la colonne selon H % 3
            for h_utc in range(24):
                horizon = HORIZON_MAP[h_utc % 3]
                feature_name = f"pred_{var}_{station}_t{h_utc:02d}"

                if horizon not in col_values:
                    features[feature_name] = None
                    continue

                idx = hour_to_idx.get(h_utc)
                if idx is None:
                    features[feature_name] = None
                    continue

                val = col_values[horizon][idx]
                features[feature_name] = float(val) if val is not None else None

    # ── Feature PV prévu J+1 : moyenne irradiance × facteur conversion
    # ANTI-LEAKAGE : basé sur pred_glob_rad h13/h14/h15, disponible avant 11h le jour J
    # Pour chaque heure H, on prend la moyenne des pred_glob_rad des 6 stations
    # et on applique le facteur de conversion pour obtenir la production PV en MW
    for h_utc in range(24):
        irr_vals = [
            features.get(f"pred_glob_rad_{station}_t{h_utc:02d}")
            for station in STATIONS
        ]
        irr_vals_clean = [v for v in irr_vals if v is not None]
        if irr_vals_clean:
            irr_mean = sum(irr_vals_clean) / len(irr_vals_clean)
            features[f"pred_pv_MW_t{h_utc:02d}"] = irr_mean * PV_SURFACE_M2 * PV_PERF_RATIO / 1_000_000
        else:
            features[f"pred_pv_MW_t{h_utc:02d}"] = None

    return features


# ─────────────────────────────────────────────
# 3. CONSTRUCTION FEATURES PAR JOUR
# ─────────────────────────────────────────────

def build_features(
    target_date: date,
    oiken: pl.DataFrame,
    meteo_utc: pl.DataFrame,
    meteo_zurich: pl.DataFrame,
    real_cols: list[str],
) -> dict | None:
    """
    Construit le vecteur de features pour prédire J+1 = target_date.
    Simule les données disponibles à 11h le jour J = target_date - 1.

    ANTI-LEAKAGE strict :
      - Load historique  : J-1 à J-7 (passé uniquement)
      - Météo réelle     : J-1 complet + J matin ≤10h (passé uniquement)
      - Prévisions météo : h13/h14/h15 sur J+1 (émises avant 11h le jour J)
      - is_ramadan_hour  : info calendaire connue à l'avance
    """
    day_j   = target_date - timedelta(days=1)   # lundi = jour de la prévision
    day_jm1 = target_date - timedelta(days=2)   # dimanche = J-1

    # ── Cible : load J+1
    oiken_target = get_day_slice(oiken, target_date)
    if len(oiken_target) < 90:
        return None

    features = {}

    # ── Load historique J-1 à J-7
    for delta in LOAD_HISTORY_DAYS:
        day_past   = target_date - timedelta(days=delta)
        label      = f"jm{delta - 1}"
        oiken_past = get_day_slice(oiken, day_past)

        if len(oiken_past) >= 90:
            features.update(series_stats(oiken_past["load"], f"load_{label}"))
            features.update(hourly_profile(oiken_past, "load", f"load_{label}"))
        else:
            for k in ["mean", "max", "min", "std"]:
                features[f"load_{label}_{k}"] = None
            for h in range(24):
                features[f"load_{label}_h{h:02d}"] = None

    # ── Production solaire : J-1 total + J matin total
    oiken_jm1       = get_day_slice(oiken, day_jm1)
    oiken_j_morning = get_morning_slice(oiken, day_j, until_hour=10)

    for col in PROD_COLS:
        if col in oiken_jm1.columns:
            features[f"{col}_jm1_total"] = float(oiken_jm1[col].sum())
        if col in oiken_j_morning.columns:
            features[f"{col}_j_morning_total"] = float(oiken_j_morning[col].sum())

    # ── Météo réelle J-1 complet (Zurich)
    # ANTI-LEAKAGE : dimanche complet, connu à 11h le lundi
    meteo_jm1 = get_day_slice(meteo_zurich, day_jm1)
    features.update(real_meteo_stats(meteo_jm1, real_cols, "rmet_jm1"))

    # ── Météo réelle J matin jusqu'à 10h (Zurich)
    # ANTI-LEAKAGE : lundi matin ≤10h, disponible avant soumission à 11h
    meteo_j_morning = get_morning_slice(meteo_zurich, day_j, until_hour=10)
    features.update(real_meteo_stats(meteo_j_morning, real_cols, "rmet_jmorn"))

    # ── Vecteur prévisions météo 24h via h13/h14/h15 (UTC)
    # ANTI-LEAKAGE : prévisions les plus récentes émises avant 11h le jour J
    features.update(pred_vector_24h(meteo_utc, target_date))

    # ── Calendaire (valeurs brutes)
    doy = target_date.timetuple().tm_yday
    features["dayofweek"]  = target_date.weekday()
    features["month"]      = target_date.month
    features["is_weekend"] = int(target_date.weekday() >= 5)
    features["is_holiday"] = int(target_date in FERIES)
    features["is_ramadan"] = int(is_ramadan(target_date))

    # ── is_ramadan_hour : 1 pour chaque heure nocturne de Ramadan
    # Feature par heure UTC de J+1 — le modèle voit quelles heures sont impactées
    # ANTI-LEAKAGE : info calendaire connue à l'avance
    ramadan_hours = set(ramadan_night_hours(target_date))
    for h in range(24):
        features[f"is_ramadan_h{h:02d}"] = int(h in ramadan_hours)

    # ── Calendaire (encodages cycliques)
    features["sin_dow"]   = math.sin(2 * math.pi * target_date.weekday() / 7)
    features["cos_dow"]   = math.cos(2 * math.pi * target_date.weekday() / 7)
    features["sin_month"] = math.sin(2 * math.pi * (target_date.month - 1) / 12)
    features["cos_month"] = math.cos(2 * math.pi * (target_date.month - 1) / 12)
    features["sin_doy"]   = math.sin(2 * math.pi * doy / 365)
    features["cos_doy"]   = math.cos(2 * math.pi * doy / 365)

    return {
        "features": features,
        "target":   oiken_target["load"].to_list(),
        "baseline": oiken_target["load_forecast_oiken"].to_list(),
        "date":     target_date,
    }


# ─────────────────────────────────────────────
# 4. PIPELINE PRINCIPAL
# ─────────────────────────────────────────────

def main():
    print("=== Chargement des données ===")
    oiken                        = load_oiken(CSV)
    meteo_utc, meteo_zurich, real_cols = load_meteo(METEO)

    first_ts  = oiken["timestamp"].drop_nulls()[0]
    first_day = first_ts.date() + timedelta(days=9)
    last_day  = oiken["timestamp"][-1].date() - timedelta(days=1)

    all_dates = [first_day + timedelta(days=i)
                 for i in range((last_day - first_day).days + 1)]

    print(f"\n=== Construction features : {first_day} → {last_day} ({len(all_dates)} jours) ===")

    rows_X, rows_Y, rows_B, dates_ok = [], [], [], []

    for i, target_date in enumerate(all_dates):
        if i % 100 == 0:
            print(f"  {i}/{len(all_dates)} — {target_date}")

        result = build_features(target_date, oiken, meteo_utc, meteo_zurich, real_cols)
        if result is None:
            continue

        rows_X.append(result["features"])
        rows_Y.append(result["target"])
        rows_B.append(result["baseline"])
        dates_ok.append(str(result["date"]))

    print(f"\n  {len(dates_ok)} jours valides sur {len(all_dates)}")

    # ── Sérialisation
    X = pl.DataFrame(rows_X).with_columns(
        pl.Series("date", dates_ok).str.strptime(pl.Date, "%Y-%m-%d")
    )
    X = X.select(["date"] + [c for c in X.columns if c != "date"])

    n_steps = len(rows_Y[0])
    Y = pl.DataFrame(
        {f"load_t{i:03d}": [row[i] if i < len(row) else None for row in rows_Y]
         for i in range(n_steps)}
    ).with_columns(pl.Series("date", dates_ok).str.strptime(pl.Date, "%Y-%m-%d"))
    Y = Y.select(["date"] + [f"load_t{i:03d}" for i in range(n_steps)])

    B = pl.DataFrame(
        {f"baseline_t{i:03d}": [row[i] if i < len(row) else None for row in rows_B]
         for i in range(n_steps)}
    ).with_columns(pl.Series("date", dates_ok).str.strptime(pl.Date, "%Y-%m-%d"))
    B = B.select(["date"] + [f"baseline_t{i:03d}" for i in range(n_steps)])

    X.write_parquet(OUT / "X_features_v5.parquet")
    Y.write_parquet(OUT / "Y_target_v5.parquet")
    B.write_parquet(OUT / "B_baseline_v5.parquet")

    print(f"\n✓ X_features_v5 : {X.shape[0]} jours × {X.shape[1]} colonnes")
    print(f"✓ Y_target_v5   : {Y.shape[0]} jours × {Y.shape[1]} colonnes")
    print(f"✓ B_baseline_v5 : {B.shape[0]} jours × {B.shape[1]} colonnes")
    print(f"✓ Sauvegardé dans : {OUT}")


if __name__ == "__main__":
    main()