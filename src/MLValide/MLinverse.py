"""
train_lgbm_v12_reverse_split.py
================================
Script de DIAGNOSTIC : split inversé pour identifier si le problème
diurne vient de la croissance PV (extrapolation temporelle) ou d'un
défaut structurel de modélisation.

Split inversé :
  - Train : 80% les plus RÉCENTS (val incluse dans les 80%)
  - Test  : 20% les plus ANCIENS

Si le modèle entraîné sur les données récentes (forte capacité PV)
prédit correctement les données anciennes (faible capacité PV) en journée,
alors le problème du split normal est bien l'incapacité de LightGBM à
extrapoler au-delà des capacités PV vues en training.

Exclut les 13-15 septembre 2025 du calcul des métriques (baseline Oiken foireuse).

Feature importance groupée par thème.

Sorties :
  DATA/models12_reverse/lgbm_t{000..095}.pkl
  DATA/models12_reverse/metrics.parquet
  DATA/models12_reverse/predictions_test.parquet
  DATA/models12_reverse/best_params_night.json
  DATA/models12_reverse/best_params_day.json
"""
...
OUT    = BASE / "models12_reverse"
OUT.mkdir(parents=True, exist_ok=True)

# Split inversé : test = 20% les plus anciens, train = 80% les plus récents
TEST_RATIO = 0.20  # premiers 20% = test
# Dans les 80% restants : 75% train + 25% val (= 60% + 20% du total)
VAL_RATIO_OF_TRAINVAL = 0.25
...
# ─────────────────────────────────────────────
# 2. SPLIT INVERSÉ : test=ancien, train=récent
# ─────────────────────────────────────────────

split_test = int(n_samples * TEST_RATIO)
# Le reste (80%) est train+val
n_trainval = n_samples - split_test
split_val_in_tv = int(n_trainval * (1 - VAL_RATIO_OF_TRAINVAL))

X_test  = X_arr[:split_test]
Y_test  = Y_arr[:split_test]
B_test  = B_arr[:split_test]
dates_test = dates[:split_test]

# Dans les 80% récents : split en train (premiers 75%) et val (derniers 25%)
X_trainval = X_arr[split_test:]
Y_trainval = Y_arr[split_test:]

X_train = X_trainval[:split_val_in_tv]
Y_train = Y_trainval[:split_val_in_tv]
X_val   = X_trainval[split_val_in_tv:]
Y_val   = Y_trainval[split_val_in_tv:]

dates_trainval = dates[split_test:]

print(f"\n=== Split INVERSÉ ===")
print(f"  Test  (ancien) : {split_test} jours ({dates[0]} → {dates[split_test-1]})")
print(f"  Train (récent) : {split_val_in_tv} jours ({dates_trainval[0]} → {dates_trainval[split_val_in_tv-1]})")
print(f"  Val   (récent) : {n_trainval - split_val_in_tv} jours ({dates_trainval[split_val_in_tv]} → {dates_trainval[-1]})")
...
print(f"  Dates exclues des métriques : {n_excluded} ({[str(d) for d in EXCLUDE_DATES]})")
...
# ─────────────────────────────────────────────
# 4. ENTRAÎNEMENT FINAL — train+val → test (ancien)
# ─────────────────────────────────────────────

print(f"\n=== Entraînement final (96 modèles, split inversé) ===")
...
for t in range(n_steps):
    ...
    mask_te = ~np.isnan(y_te) & exclude_mask  # exclure dates problématiques

    dtrain = lgb.Dataset(X_tv[mask_tv], label=y_tv[mask_tv],
                         feature_name=feat_names, free_raw_data=False)

    # Pour early stopping : utiliser une portion du trainval comme proxy
    # (pas le test set — c'est les données anciennes)
    dval_es = lgb.Dataset(X_val[~np.isnan(Y_val[:, t])],
                          label=Y_val[:, t][~np.isnan(Y_val[:, t])],
                          reference=dtrain, free_raw_data=False)
...
print(f"\n=== Résultats globaux (split INVERSÉ, excl. 13-15 sept) ===")
...
# Par tranche horaire
print(f"\n=== RMSE par tranche horaire (split INVERSÉ) ===")
...
dates_arr = np.array(dates_test_list)
...
for i, d in enumerate(dates_test_list):
    ...
    if mask.sum() == 0:
        continue
    bias = float(np.mean(y_day[mask] - p_day[mask]))  # réel - prédit
    # On accumule par mois
    key = f"{d.year}-{d.month:02d}"

# Refaire proprement par mois
